...
public class StructurePopulatorBiomeCenter extends BlockPopulator {
    ...
    public StructurePopulatorBiomeCenter(boolean surfaceOrUnderground) {
        super();
    ...
    static int getBlockCoordByChunk(int chunkCoord) {
        return (chunkCoord << 4) + 8;
    }

    static int getSurfaceY(World wld, int x, int z, boolean ignoreWater) {
        Block highestBlk = wld.getHighestBlockAt(x, z);
        boolean invalid = true;
        while (invalid && highestBlk.getY() >= 1) {
            ...
            if (invalid) highestBlk = highestBlk.getRelative(BlockFace.DOWN);
        }
    ...
    static Location getCavernOpening(World wld, int centerX, int centerZ,
        ...
        int x, z;
        for (int tol = 0; tol <= xzToleranceSteps * xzToleranceStepSize; tol += xzToleranceStepSize) {
            searchXZ.clear();
            for (int offset = -tol; offset <= tol; offset += xzToleranceStepSize) {
                searchXZ.add(new Location(wld, centerX - tol, 0, centerZ + offset));
                searchXZ.add(new Location(wld, centerX + tol, 0, centerZ + offset));
                searchXZ.add(new Location(wld, centerX + offset, 0, centerZ - tol));
                searchXZ.add(new Location(wld, centerX + offset, 0, centerZ + tol));
            }
            for (Location loc : searchXZ) {
                x = loc.getBlockX();
                z = loc.getBlockZ();
                for (int y = yMax; y >= yMin; y -= yStep) {
                    if (wld.getBlockAt(x, y, z).getType().isSolid()) continue;
...
//            if (! validYs.isEmpty()) break;
    ...
    // jungle lizard temple
    protected void generateLizardTemple(World wld, int blockX, int blockZ) {
        // try to make the temple visible in some cavern structure
        Location loc = getCavernOpening(wld, blockX, blockZ, 4, 4, 50, 175, 5);
        blockX = loc.getBlockX();
        int startY = loc.getBlockY() - 20;
