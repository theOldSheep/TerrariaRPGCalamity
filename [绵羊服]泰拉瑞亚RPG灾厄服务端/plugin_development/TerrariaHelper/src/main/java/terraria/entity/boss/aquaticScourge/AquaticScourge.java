...
public class AquaticScourge extends EntitySlime {
    ...
    private void shootProjectiles(int attackMethod) {
        ...
        switch (attackMethod) {
            // tooth
            case 1: {
                shootInfo.velocity = MathHelper.getDirection(target.getEyeLocation(), shootInfo.shootLoc, 2);
            ...
            case 3: {
                for (Vector direction : MathHelper.getCircularProjectileDirections(
                        8, 3, 180, target, shootInfo.shootLoc, 1.25)) {
    ...
    public AquaticScourge(Player summonedPlayer, ArrayList<LivingEntity> bossParts, int index) {
        ...
        // boss parts and other properties
        {
            ...
            // projectile info
            projectilePropertySandTooth = new EntityHelper.ProjectileShootInfo(bukkitEntity, new Vector(), attrMap, "Arrow", "渊海灾虫毒牙");
            projectilePropertySandPoisonCloud = new EntityHelper.ProjectileShootInfo(bukkitEntity, new Vector(), attrMap, "Arrow", "渊海灾虫沙爆");
            projectilePropertyToxicCloud = new EntityHelper.ProjectileShootInfo(bukkitEntity, new Vector(), attrMap, "Arrow", "渊海灾虫毒云");
            // segment settings
